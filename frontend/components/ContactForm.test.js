...
test('renders the contact form header', () => {
    ...
    const headerElement = screen.findByText(/Contact Form/i);
...
test('renders ONE error message if user enters a valid first name and last name but no email.', async () => {
    ...
    const firstNameField = screen.getByLabelText(/First Name*/i);
        userEvent.type(firstNameField, "sydney");

    const lastNameField = screen.getByLabelText(/Last Name*/i);
        userEvent.type(lastNameField, "hopson");
...
test('renders "email must be a valid email address" if an invalid email is entered', async () => {
    ...
    userEvent.type(emailField, "sydney@gmail");
...
test('renders all firstName, lastName and email text when submitted. Does NOT render message if message is not submitted.', async () => {
    render(<ContactForm/>);
        ...
        userEvent.type(firstNameField, "sydney");
        userEvent.type(lastNameField, "hopson");
        userEvent.type(emailField, "sydney@email.com");

        const button = screen.getByRole("button");
        userEvent.click(button); 
        await waitFor(()=> {
        const firstNameDisplay = screen.queryAllByText("sydney");
        const lastNameDisplay = screen.queryAllByText("hopson");
        const emailDisplay = screen.queryAllByText("sydney@email.com");
